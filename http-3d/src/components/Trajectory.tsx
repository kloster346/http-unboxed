...
/** 数据球飞行的发光轨迹：橙 = 去程(手机→快递站→仓库)，绿 = 回程(仓库→手机，与 200 OK 签收球同色)。 */
export default function Trajectory() {
  ...
  return (
    <>
      <Line points={toPoints(outCurve)} color="#fb923c" lineWidth={2.5} transparent opacity={0.7} />
      <Line points={toPoints(retCurve)} color="#4ade80" lineWidth={2.5} transparent opacity={0.55} />
    </>
  );
}
